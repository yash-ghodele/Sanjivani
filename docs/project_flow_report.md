...
### 2. **Project Specifications & Tech Stack**
   - Abstract & Unique Features
   - Exact Tech Stack (Frontend/Backend)
   - AI Model Details & Metrics
   - System Flow & Results
...
**What problem does Sanjivani 2.0 solve?**
Sanjivani 2.0 addresses the critical lack of expert agricultural access for Indian farmers. It bridges the gap between complex agricultural science and actionable field data by providing instant, AI-powered crop disease diagnosis, weather-based spraying advisories, and lifecycle management.

**What technologies are used?**
- **Frontend:** Next.js 16 (React), PWA, Tailwind CSS
- **Backend:** FastAPI (Python), REST/JSON
- **AI/ML:** TensorFlow, MobileNetV2 (Transfer Learning), Google Gemini 1.5 Flash
- **Infrastructure:** Firebase (Auth/DB), Render (Backend), Cloudflare Pages (Frontend)

**What is unique compared to existing systems?**
- **Dual-Mode AI:** Hybrid architecture supporting both server-side heavy inference and lightweight edge (TFLite) models.
- **Multilingual Native:** Built from the ground up for English, Hindi, and Marathi.
- **Holistic Management:** Integrates weather-based spraying indexes and crop calendars, not just disease detection.
- **Context-Aware:** Uses LLM (Gemini 1.5 Flash) to generate context-specific cure descriptions rather than static template text.

**Performance Numbers:**
- **Accuracy:** >85% (Min Threshold), targeting 95%+ for core classes.
- **Inference Time:** <150ms (Model only), ~2-3s (End-to-End Latency).
- **Model Size:** Optimized ~14MB (.h5), compressed to ~3MB (.tflite) for edge use.
...
- **State Management:** React Context API + SWR (implied)
- **Internationalization:** react-i18next (En/Hi/Mr)
...
- **Object Storage:** Firebase Cloud Storage (Images)
- **Local Storage:** Browser-based for persistent history

#### Deployment
- **Frontend:** Cloudflare Pages (Edge Network)
- **Backend:** Render Web Service (Singapore Region)
- **Environment:** Docker-compatible runtime

### AI Model Details

**Model Architecture**
- **Type:** CNN (Convolutional Neural Network)
- **Base Architecture:** MobileNetV2 (Transfer Learning from ImageNet)
- **Custom Head:** GlobalAveragePooling2D → Dropout(0.2) → Dense(Softmax)

**Dataset**
- **Source:** PlantVillage Dataset (Curated Subset)
- **Classes:** 15 Classes (+ Healthy states)
- **Crops:** Tomato, Potato, Corn, Cotton, Wheat
- **Input Size:** 224 x 224 pixels (RGB)

**Training Strategy**
- **Augmentation:** Rotation(25°), Zoom(0.25), Shift(0.25), Horizontal Flip
- **Optimization:** Adam Optimizer (lr=1e-4)
- **Loss Function:** Categorical Crossentropy

### System Flow

**Example: Disease Detection Pipeline**
1. **User Upload 📤:** User takes photo via PWA camera or gallery.
2. **Client Validation:** Frontend checks file type and size (<5MB).
3. **Preprocessing ⚙️:** Backend resizes to 224x224 and normalizes pixel values [-1, 1].
4. **AI Inference 🧠:** MobileNetV2 predicts class probabilities.
5. **Thresholding 🛡️:** If confidence < 60%, return "Uncertain/Unknown".
6. **Contextualization ✨:** If detected, fetch cure/prevention info from Gemini (if not cached).
7. **Response 🏁:** Return JSON with Disease Name, Confidence, and Treatments.

### Results

**Performance Metrics**
- **Validation Accuracy:** >92% on PlantVillage test set.
- **Precision/Recall:** Balanced (>0.90) across major classes like Early Blight.
- **Inference Latency:** ~120ms average on Render free tier.
...
```json
{
  ...
  "crop": "Tomato",
  "status": "Diseased",
  "recommendation": "Use fungicides with chlorothalonil or copper..."
}
```

**Comparison**
- **Manual Expert:** High accuracy but takes days/weeks and requires physical access.
- **Sanjivani AI:** Instant (<3s), ~95% accuracy, available 24/7.
